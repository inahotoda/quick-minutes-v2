...
└─────────────────────────────────────────────────────────────┘
  ...
  【会議室での利用（基本）】
     ...
     🖥️ Mac + 🎙️ 会議用マイク（Anker PowerConf S3など）
     
        ┌─────────────┐
        │   Mac      │ ← Chromeでアプリ化してDockに追加
        └──────┬──────┘
               │ USB-C接続
        ┌──────┴──────┐
        │  🎙️ マイク  │ ← テーブルの中央に設置
...
| シーン | 推奨デバイス | 備考 |
|--------|-------------|------|
| **会議室で複数人** | Mac + 会議用マイク | 議事録確認を大画面でできる |
| **外出先・突発的な会議** | iPhone | 手軽にすぐ録音できる |

#### Macでのセットアップ手順

1. **Google Chrome**でアプリにアクセス
2. 右上の「⋮」メニュー →「保存して共有」→「ショートカットを作成」
3. 「ウィンドウとして開く」にチェックを入れて作成
4. Dockに表示されたアイコンを**右クリック** →「オプション」→「Dockに残す」

> 💡 **ポイント**: 一度セットアップすれば、DockからワンクリックでアプリをOpen できます。

---

## 📲 ホーム画面に追加（おすすめ）

すぐにアプリを起動できるよう、**ホーム画面への追加を推奨**します。
...
| 設定 | 説明 | おすすめの名前 |
|------|------|---------------|
| 「Webアプリとして開く」**OFF** | 通常のブラウザで開く（バックグラウンド録音OK） | 議事録 |
| 「Webアプリとして開く」**ON** | PWAモード（高速起動・キャッシュ活用） | 議事録(PWA) |

> 💡 **おすすめ**: 両方をホーム画面に追加して使い分けましょう！
> - **通常版**: 長時間の会議や他のアプリを使いながら録音したい時
> - **PWA版**: 電波が不安定な場所での利用、素早く起動したい時
...
2. 「メンバー管理」セクションへ
3. 「+ 新しいメンバーを追加」をタップ
4. **🎤 音声を録音**（10秒程度、自分の名前や自己紹介を話す）
5. 名前が自動で入力されるので、確認して保存

> 💡 **ポイント**: 音声を録音すると、AIが自己紹介から名前を自動認識します。静かな場所で、はっきりと録音してください。
...
| モード | 説明 | 議事録の特徴 |
|--------|------|-------------|
| **🏢 社内** | 社内ミーティング用 | 決定事項・アクションアイテム重視 + **スコアリング機能** |
| **🤝 商談** | お客様との商談用 | 顧客要望・次回アクション重視 |
| **📁 その他** | 汎用的な会議用 | バランスの取れた構成 |

> 📊 **社内モードのスコアリング機能**  
> 会議全体の**会議スコア**と、各参加者の**貫献度スコア**が自動で算出されます。  
> 会議の質や参加度を客観的に振り返るのに役立ちます。

**選び方**: 録音画面で該当するモードのアイコンをタップするだけ！

---

### 📋 会議プリセット

よく開催する会議の設定を **事前に保存** しておける便利機能です。
...
| 保存できる設定 | 説明 |
|---------------|------|
| **プリセット名** | 「週次定例」「営業チームMtg」など |
| **会議モード** | 社内 / 商談 / その他 |
| **参加メンバー** | 事前登録したメンバーから選択 |
| **会議時間** | 30分 / 60分 / 無制限 |
...
### 🔐 Google Driveへのアクセス権について

このアプリを利用するには、議事録保存用の**共有ドライブへのアクセス権**が必要です。
...
└─────────────────────────────────────────────────────────────┘
  ...
  ① ログイン後、アクセス権がない場合は案内が表示される
  
  ② 表示されるリンクから共有ドライブにアクセス
  
  ③ 「アクセス権をリクエスト」をクリック
  
  ④ 管理者の承認後、アプリを再読み込みして利用開始
```

> 💡 **ポイント**: 初回利用時のみ必要な手続きです。一度アクセス権が付与されれば、以降は自動的にログインできます。

---

### 📱 PWA（プログレッシブWebアプリ）について

このアプリはPWAに対応していますが、**Googleログインが必須**のため、完全なオフライン利用はできません。

| シナリオ | 通常ブラウザ | PWA |
|---------|-------------|-----|
| **電波が不安定** | 読み込みが遅い・失敗 | キャッシュで高速表示 ✅ |
| **ログイン済みでオフライン** | 操作不可 | UI表示＋録音は可能 ✅ |
| **ホーム画面からアクセス** | URLを入力 | アプリアイコンから起動 ✅ |
| **完全オフライン（未ログイン）** | 利用不可 | 利用不可（ログイン必須） |

> 💡 **おすすめの使い方**: 電波が弱い場所（地下会議室など）では、**事前にログインした状態でアプリを開いておく**と安定して使えます。録音はローカルで動作するので問題ありません。
